...
load(file.path(outputfolder,"out",
               paste(samplename,".DataMatrix.RData", sep = "")))
load(file.path(outputfolder,"out",
               paste(samplename,".ids_homology.RData", sep  = "")))
...
load(file.path(ericscriptfolder, "lib","data",
               "_resources", "BlackList.RData"))
load(file.path(ericscriptfolder, "lib","data",
               "_resources", "DataModel.RData"))

genomeref <- file.path(dbfolder, "data", refid, "allseq.fa")
checkselfhomology.fa <- scan(file = file.path(outputfolder,"out",
                        paste(samplename, ".checkselfhomology.fa", sep = "")),
                        sep= "\n", what = "", quiet = T)

seq.length <- nchar(checkselfhomology.fa)[2]
ix.id.fa   <- which(checkselfhomology.fa %in% 
                    paste(">",info.id.and.homology[,1], sep = ""))
ix.fa      <- ix.id.fa + 1
left_junction  <- substr(checkselfhomology.fa[ix.fa], 1, (seq.length/2))
right_junction <- substr(checkselfhomology.fa[ix.fa],
                        (seq.length/2 + 1), seq.length)
...
ensgenename1 <- unlist(strsplit(exone.tmp1, "_"))[seq(1, dim(DataMatrix)[1]*2,
                                                  by = 2)]
ensgenename2 <- unlist(strsplit(exone.tmp2, "_"))[seq(1, dim(DataMatrix)[1]*2,
                                                  by = 2)]
...
exone.tmp    <- unlist(strsplit(as.character(DataMatrixF[,1]),
                                "----", fixed = T))
exone.tmp1   <- exone.tmp[seq(1, dim(DataMatrixF)[1]*2, by = 2)]
exone.tmp2   <- exone.tmp[seq(2, dim(DataMatrixF)[1]*2, by = 2)]
ensgenename1 <- unlist(strsplit(exone.tmp1, "_"))[seq(1, dim(DataMatrixF)[1]*2,
                                                  by = 2)]
ensgenename2 <- unlist(strsplit(exone.tmp2, "_"))[seq(1, dim(DataMatrixF)[1]*2,
                                                  by = 2)]
...
exone1 <- unlist(strsplit(exone.tmp1, "_"))[seq(2, dim(DataMatrixF)[1]*2,
                                                by = 2)]
exone2 <- unlist(strsplit(exone.tmp2, "_"))[seq(2, dim(DataMatrixF)[1]*2,
                                                by = 2)]
genename1 <- geneid1
genename2 <- geneid2
...
if (refid == "homo_sapiens") { 
  ...
  ix.chr.tmp2 <- intersect(grep("NNN", right_junction, invert = T),
                           grep("NNN", left_junction, invert = T))
...
stats <- read.delim(file.path(outputfolder, "out",
                              paste(samplename, ".stats", sep = "")),
                              sep = "\t", header = F)
...
gene12.exp   <- round(((as.numeric(DataMatrixF[,2]) +
                        as.numeric(DataMatrixF[,4])) * readlength /
                        as.numeric(DataMatrixF[,5])), digits = 2)
...
for (iexpr in 1: length(ensgenename1)) {
  ...
  gene1.exp[iexpr] <- round(nreads.stats[ix1.stats] * readlength /
                            length.stats[ix1.stats], digits = 2)
  gene2.exp[iexpr] <- round(nreads.stats[ix2.stats] * readlength /
                            length.stats[ix2.stats], digits = 2)
  #    gene12.exp[iexpr] <- round(nreads.stats[ix12.stats] * readlength /
  #                               length.stats[ix12.stats], digits = 2)
}
...
for (i in 1: length(ensgenename1)) {
  if (i == 1) {
    cat(paste("@", i, "_", 1, "\n", left_junction[i],
              "\n+\n", gsub(", ", "",
              toString(rep("I", nchar(left_junction[i])))), "\n", "@", i,
              "_", 2, "\n", right_junction[i], "\n+\n",
              gsub(", ", "", toString(rep("I", nchar(right_junction[i])))),
              sep = ""), sep = "\n",
              file = file.path(outputfolder, "out", "findgenomicpos.fq"),
              append = F)    
  } else {
    cat(paste("@", i, "_", 1, "\n", left_junction[i],
              "\n+\n", gsub(", ", "",
...
system(paste("bwa aln", "-R 50", genomeref,
             file.path(outputfolder, "out", "findgenomicpos.fq"), ">",
             file.path(outputfolder, "out", "findgenomicpos.fq.sai"), "2>>",
             file.path(outputfolder, "out", ".ericscript.log")))
system(paste("bwa samse", "-n 50", genomeref,
             file.path(outputfolder, "out", "findgenomicpos.fq.sai"),
             file.path(outputfolder, "out", "findgenomicpos.fq"), ">",
             file.path(outputfolder, "out", "findgenomicpos.fq.tmp"), "2>>",
             file.path(outputfolder, "out", ".ericscript.log")))
system(paste("cat", file.path(outputfolder, "out", "findgenomicpos.fq.tmp"),
             "|", file.path(ericscriptfolder, "lib", "perl", "xa2multi.pl"),
             "-", "|","grep -v -e \'^\\@\' -",">",
             file.path(outputfolder, "out", "findgenomicpos.fq.sam")))
xx.pos <- read.delim(file.path(outputfolder, "out", "findgenomicpos.fq.sam"),
                     sep = "\t", header= F)
...
for (i in 1: length(ensgenename1)) {
  ...
  ix.okpos    <- which(chr.pos.ix == chr1[i] & pos.pos.ix >=
                       as.numeric(genestart1[i]) & pos.pos.ix <=
                       as.numeric(geneend1[i]))
  ...
  pos.pos.ix  <- pos.pos[ix.mypos]
  mapq.pos.ix <- mapq.pos[ix.mypos] 
  ix.okpos    <- which(chr.pos.ix == chr2[i] & pos.pos.ix >=
                       as.numeric(genestart2[i]) & pos.pos.ix <=
                       as.numeric(geneend2[i]))
  if (length(ix.okpos) > 1) {
    ix.okpos  <- ix.okpos[which.max(mapq.pos.ix[ix.okpos])]
  }
  if (length(ix.okpos) > 0) {
...
if (length(ix.na.pos_1 ) > 0 | length(ix.na.pos_2 ) > 0) {
    ...
    for (i in 1: length(ensgenename1)) {
        if (i == 1) {
            cat(paste("@", i, "_", 1, "\n", left_junction.trim[i], "\n+\n",
                gsub(", ", "",
                     toString(rep("I", nchar(left_junction.trim[i])))),
                "\n", "@", i, "_", 2, "\n", right_junction.trim[i],"\n+\n",
                gsub(", ", "",
                     toString(rep("I", nchar(right_junction.trim[i])))),
                sep = ""), sep = "\n",
                file = file.path(outputfolder, "out", "findgenomicpos.fq"),
                append = F)    
        } else {
    ...
    system(paste("bwa aln", "-R 50", genomeref,
                 file.path(outputfolder, "out", "findgenomicpos.fq"), ">",
                 file.path(outputfolder, "out", "findgenomicpos.fq.sai"),
                 "2>>", file.path(outputfolder, "out", ".ericscript.log")))
    system(paste("bwa samse", "-n 50", genomeref,
                 file.path(outputfolder, "out", "findgenomicpos.fq.sai"),
                 file.path(outputfolder, "out", "findgenomicpos.fq"), ">",
                 file.path(outputfolder, "out", "findgenomicpos.fq.tmp"),
                 "2>>", file.path(outputfolder, "out", ".ericscript.log")))
    system(paste("cat",
                 file.path(outputfolder, "out", "findgenomicpos.fq.tmp"), "|",
                 file.path(ericscriptfolder, "lib", "perl", "xa2multi.pl"),
                 "-", "|","grep -v -e \'^\\@\' -",">",
                 file.path(outputfolder, "out", "findgenomicpos.fq.sam")))
    xx.pos   <- read.delim(file.path(outputfolder, "out",
                                     "findgenomicpos.fq.sam"),
                           sep = "\t", header= F)
    id.pos   <- as.character(xx.pos[[1]])
    flag.pos <- as.character(xx.pos[[2]])
    chr.pos  <- as.character(xx.pos[[3]])
...
#   if (strand1[i] == "+") {
#     exonpos <- as.numeric(unlist(strsplit(as.character(
#                           EnsemblGene.Structures$exonEnd[ix.ref.table]),
#                           ",")))
#   } else {
#     exonpos <- as.numeric(unlist(strsplit(as.character(
#                           EnsemblGene.Structures$exonStart[ix.ref.table]),
#                           ",")))
#   }
...
if (flag.ada) {
  ...
  sig   <- sigest(control~., data = DataScores, frac = 1, na.action = na.omit,
                  scaled = TRUE)[2]
  model <- ksvm(control~., data = DataScores, type = "C-svc",
                kernel = "rbfdot", kpar = list(sigma = sig), C = 1,
                prob.model = TRUE)
  ericscore <- predict(model, myscores, type = "probabilities")[,2]  
}
ix.repeated   <- unique(c(which(is.na(genpos_1)), which(is.na(genpos_2))))
ix.norepeated <- intersect(which(is.na(genpos_1) == F),
                           which(is.na(genpos_2) == F))
genpos_1.recal[which(genpos_1.recal == 0)] <- 
    "Unable to predict breakpoint position"
genpos_2.recal[which(genpos_2.recal == 0)] <- 
    "Unable to predict breakpoint position"
myblacklist <- rep("", length(genename1))
ix.bl <- which((genename1 %in% gene.bl1 & genename2 %in% gene.bl2) |
               (genename1 %in% gene.bl2 & genename2 %in% gene.bl1))
if (length(ix.bl) > 0) {
  for (bli in 1: length(ix.bl)) {
    ix.bli <- which((gene.bl1 == genename1[ix.bl[bli]] &
                     gene.bl2 == genename2[ix.bl[bli]]) |
                    (gene.bl2 == genename1[ix.bl[bli]] &
                     gene.bl1 == genename2[ix.bl[bli]]))
...
fusion.type[intersect(intersect(which(chr1==chr2), which(adj == "Adjacent")),
                                which(strand1 == strand2))] <- "Read-Through"
fusion.type[intersect(intersect(which(chr1==chr2), which(adj == "Adjacent")),
                                which(strand1 != strand2))] <- "Cis"
junctionsequence <- paste(tolower(left_junction), right_junction, sep = "")
SummaryMat <- cbind(genename1, genename2, chr1, genpos_1.recal, strand1, chr2,
                    genpos_2.recal , strand2, ensgenename1, ensgenename2,
                    n.crossing, n.spanning,ins.size, homology, fusion.type,
                    myblacklist,description1, description2, junctionsequence,
                    gene1.exp, gene2.exp, gene12.exp, edge.score, gjs.score,
                    nreads.score, ericscore)
colnames(SummaryMat) <- c("GeneName1", "GeneName2", "chr1", "Breakpoint1",
                          "strand1", "chr2", "Breakpoint2", "strand2",
                          "EnsemblGene1", "EnsemblGene2", "crossingreads",
                          "spanningreads", "mean insertsize", "homology",
                          "fusiontype", "Blacklist", "InfoGene1", "InfoGene2",
                          "JunctionSequence", "GeneExpr1", "GeneExpr2",
                          "GeneExpr_Fused", "ES", "GJS", "US", "EricScore")
SummaryMat <- data.frame(SummaryMat)
save(SummaryMat, file=file.path(outputfolder,
     paste(samplename, ".Summary.RData", sep = "")))
if (dim(SummaryMat)[1] > 0) {
    write.table(SummaryMat, file = file.path(outputfolder,
                paste(samplename,".results.total.tsv", sep = "")),
    ...
    SummaryMat.Filtered <-  SummaryMat.sorted[which(ericscore.sorted > 0.5 &
                            myblacklist.sorted == "" &
                            oddity.spanningreads.sorted == 0), ]
    write.table(SummaryMat.Filtered[, -16],
                file = file.path(outputfolder,
                paste(samplename,".results.filtered.tsv", sep = "")),
                sep = "\t", row.names = F, quote = F)
} else {
  Results <- "No Chimeric Transcript found!"
  write.table(Results, file = file.path(outputfolder,
              paste(samplename,".results.total.tsv", sep = "")),
