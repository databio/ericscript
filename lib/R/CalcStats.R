...
algonamelist <- c("ericscript", "chimerascan", "defuse",
                  "fusionmap", "shortfuse")
...
nsims <- length(xx)
cat("[EricScript calcstats] Found ", nsims,
...
if (nsims > 0) {
	...
	for (i in 1: nsims) {
		if (i < 10) {
			dataresults <- (get(paste("Import", algoid, sep = "_"))
                           (file.path(resultsfolder,
                                      paste("sim_", "0000", i, sep = ""))))
			if (is.list(dataresults)) {
				load(file.path(refpath, paste("sim_", "0000", i, sep = ""),
                     "GeneFusions.RData"))
				cat ("[EricScript calcstats] Analysing ",
                     paste("sim_", "0000", i, sep = "")," ... ")
			} else if (dataresults == 0) {
				cat("[EricScript calcstats] No results file found for ",
                    paste("sim_", "0000", i, sep = ""), ".\n", sep = "")
				nosims <- nosims + 1
				next
			} else if (dataresults > 1) {
				cat("[EricScript calcstats] Error: ", dataresults,
                    " results file found for ",
                    paste("sim_", "0000", i, sep = ""),
                    ". Only 1 file of results is required. \n", sep = "")
				nosims <- nosims + 1
				next
			}
		} else if (i >= 10 & i < 100) {
			dataresults <- (get(paste("Import", algoid, sep = "_"))
                           (file.path(resultsfolder,
                                      paste("sim_", "000", i, sep = ""))))
			if (is.list(dataresults)) {
				load(file.path(refpath, paste("sim_", "000", i, sep = ""),
                     "GeneFusions.RData"))
				cat ("[EricScript calcstats] Analysing ",
                     paste("sim_", "000", i, sep = "")," ... ")
			} else if (dataresults == 0) {
			...
			dataresults <- (get(paste("Import", algoid, sep = "_"))
                           (file.path(resultsfolder,
                                      paste("sim_", "00", i, sep = ""))))
			if (is.list(dataresults)) {
				load(file.path(refpath, paste("sim_", "00", i, sep = ""),
                     "GeneFusions.RData"))
				cat ("[EricScript calcstats] Analysing ",
                     paste("sim_", "00", i, sep = "")," ... ")
			} else if (dataresults == 0) {
			...
			dataresults <- (get(paste("Import", algoid, sep = "_"))
                           (file.path(resultsfolder,
                                      paste("sim_", "0", i, sep = ""))))
			if (is.list(dataresults)) {
				load(file.path(refpath, paste("sim_", "0", i, sep = ""),
                     "GeneFusions.RData"))
				cat ("[EricScript calcstats] Analysing ",
                     paste("sim_", "0000", i, sep = "")," ... ")
			} else if (dataresults == 0) {
			...
			dataresults <- (get(paste("Import", algoid, sep = "_"))
                           (file.path(resultsfolder,
                                      paste("sim_", i, sep = ""))))
			if (is.list(dataresults)) {
				load(file.path(refpath, paste("sim_", i, sep = ""),
                     "GeneFusions.RData"))
				cat ("[EricScript calcstats] Analysing ",
                     paste("sim_",  i, sep = "")," ... ")
			} else if (dataresults == 0) {
		...
		tpr[i] <- (length(which(gene1ens %in% id1.simul &
                                gene2ens %in% id2.simul)) /
                                ngenefusions)
		ix.tpr <- which(id1.simul %in% gene1ens & id2.simul %in%gene2ens)
		ix.fpr <- which((gene1ens %in% id1.simul &
                         gene2ens %in% id2.simul) == F)
		cov.tpr[ix.tpr] <- cov.tpr[ix.tpr] + 1
		
		tpr.5[i] <- (length(which(gene1ens %in% id1.simul &
                                  gene2ens %in% id2.simul & nreads > 5)) /
                                  ngenefusions)
		fpr.5[i] <- (length(which((gene1ens %in% id1.simul &
                                   gene2ens %in% id2.simul) == F &
                                   nreads > 5)) / 
                                   length(gene1ens))
		...
		rocs.tpr <- colSums(rbind(rocs.tpr,
                        tabulate(score[which(gene1ens %in% id1.simul &
                                             gene2ens %in% id2.simul)] * 1000,
                                             nbins = 1000)))
		rocs.fpr <- colSums(rbind(rocs.fpr,
                        tabulate(score[which((gene1ens %in% id1.simul &
                                              gene2ens %in% id2.simul) == F)] *
                                              1000, nbins = 1000)))
		...
		for (ii in 1: length(ix.tpr)) {
			ix.tmp <- which(gene1ens == id1.simul[ix.tpr[ii]] &
                            gene2ens == id2.simul[ix.tpr[ii]])
			if (length(ix.tmp) > 1) { 
				ix.tmp <- ix.tmp[1]
			}
			if (length(agrep(toupper(seq[ix.tmp]),
                       seq.simul[ix.tpr], max = 5)) > 0 &
                       is.na(seq[ix.tmp]) == F) {
	...
	save(stats, file = file.path(outputfolder,
         paste(algoname, dataset, readlength, "stats","RData", sep = ".")))
} else {
	cat ("[EricScript calcstats] Error: no directories containing results on",
         " synthetic data have been found in ", resultsfolder, ". Exit.\n",
         sep = "")
}
