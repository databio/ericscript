...
x.bed <- read.delim(file.path(tmpfolder, "exonstartend.mrg.txt"),
                    sep = "\t", header = F)
refid.bed <- paste(as.character(x.bed[[1]]),
                   paste((as.numeric(as.character(x.bed[[2]])) + 1),
                   as.character(x.bed[[3]]), sep = "-"), sep = ":")
tmp <- grep(">", x)
...
sequences.tmp[length(tmp)] <-
    gsub(", ", "", toString(x[(tmp[length(tmp)] + 1): length(x)]))
genenames.tmp    <- as.character(x.bed[[4]])
unique.genenames <- unique(genenames.tmp)
strand.tmp       <- read.delim(file.path(tmpfolder, "strand.txt"),
                               sep = "\t", header = F)
strand    <- strand.tmp[[2]]
...
for (i in 1: length(unique.genenames)) {
    ...
    if (i == 1) {
      cat(genenames1, 
          file = file.path(refid.folder, "EnsemblGene.Reference.fa"),
          append = F, sep = "\n")
    } else {
      cat(genenames1,
          file = file.path(refid.folder, "EnsemblGene.Reference.fa"),
