## re-calculate breakpoints positions for samples analyzed with
## ericscript < 0.4.0 and re-estimation of ericscore and blacklist
...
if (length(myls) == 1 & length(myls.tsv.total) == 1 & length(myls.tsv.filt)) {
  ...
  cat(paste("[EricScript] Re-estimating EricScore for sample ",
            samplename, "... ", sep = "")) 
  ...
  if (flag.ada) {
    ...
    sig   <- sigest(control~., data = DataScores, frac = 1,
                    na.action = na.omit, scaled = TRUE)[2]
  ...
  cat("done. \n")
  cat(paste("[EricScript] Re-calculating breakpoint positions for sample ",
            samplename, "... ", sep = ""))
  ...
  system(paste("bwa samse", "-n 50", genomeref, file.path(outputfolder, "out",
               "findgenomicpos.fq.sai"),
               file.path(outputfolder, "out", "findgenomicpos.fq"), ">",
  ...
  for (i in 1: length(ensgenename1)) {
    if (i %in% ix.na.pos_1) {
      ...
      pos.pos.ix  <- pos.pos[ix.mypos]
      mapq.pos.ix <- mapq.pos[ix.mypos] 
      ix.okpos    <- which(chr.pos.ix == chr1[i] &
                           pos.pos.ix >= as.numeric(genestart1[i]) &
                           pos.pos.ix <= as.numeric(geneend1[i]))
      if (length(ix.okpos) > 1) {
        ix.okpos  <- ix.okpos[which.max(mapq.pos.ix[ix.okpos])]
      }
      if (length(ix.okpos) > 0) {
    ...
    if (i %in% ix.na.pos_2) {
      ...
      pos.pos.ix  <- pos.pos[ix.mypos]
      mapq.pos.ix <- mapq.pos[ix.mypos] 
      ix.okpos    <- which(chr.pos.ix == chr2[i] &
                           pos.pos.ix >= as.numeric(genestart2[i]) &
                           pos.pos.ix <= as.numeric(geneend2[i]))
      if (length(ix.okpos) > 1) {
        ix.okpos  <- ix.okpos[which.max(mapq.pos.ix[ix.okpos])]
      }
      if (length(ix.okpos) > 0) {
...
#       exonpos <- as.numeric(
#           unlist(strsplit(
#               as.character(EnsemblGene.Structures$exonEnd[ix.ref]), ",")))
#     } else {
#       exonpos <- as.numeric(
#           unlist(strsplit(
#               as.character(EnsemblGene.Structures$exonStart[ix.ref]), ",")))
#     }
...
#   SummaryMat$Breakpoint2 <- genpos_2.recal
  ...
  save(SummaryMat,
       file=file.path(outputfolder,
                      paste(samplename, ".Summary.recalc.RData", sep = "")))
  ...
  if (dim(SummaryMat)[1] > 0) {
    write.table(SummaryMat,
                file = file.path(outputfolder,
                        paste(samplename,".results.recalc.total.tsv",
                              sep = "")),
    ...
    write.table(SummaryMat.Filtered[, -16],
                file = file.path(outputfolder,
                        paste(samplename,".results.recalc.filtered.tsv",
                              sep = "")),
                sep = "\t", row.names = F, quote = F)
  }
  cat("done. \n")
  cat(paste("[EricScript] Breakpoint position corrected files are in ",
            outputfolder, ".\n", sep = ""))
  
} else {  
  cat("[EricScript] No files of results found in ", outputfolder,
