...
mysyndata <- file.exists(file.path(dbfolder, "data", refid,
                                   "EnsemblGene.Transcripts.RData"))
if (mysyndata == T) {
  cat("[EricScript simulator] Load genes data ...")
  load(file.path(dbfolder, "data", refid, "EnsemblGene.Transcripts.RData"))
} else {
  cat( paste("[EricScript simulator] You need to download", refid,
             "data before running EricScript Simulator. Exit.\n"))
...
# 	download.file(myurl, destfile = file.path(dbfolder, "data",
#                                             "EnsemblGene.Transcripts.RData"),
#                                             quiet = T)
# 	load(file.path(dbfolder, "data", "EnsemblGene.Transcripts.RData"))
# 	cat(" done.\n")
...
ix.goodseq <- which((nchar(sequences) > 2*minlength) &
                    (TranscriptNames %in% genenameok) &
                     is.na(GeneNames) == F)
...
formatted.count <- substr(formatted.count.tmp, nchar(formatted.count.tmp) - 4,
                          nchar(formatted.count.tmp))
formatted.count.tmp <-paste("00000", seq(1, ngenefusion), sep = "")
formatted.count.fusions <- substr(formatted.count.tmp,
                                  nchar(formatted.count.tmp) - 4,
                                  nchar(formatted.count.tmp))
...
for (tt in 1: length(dataset)) {
    ...
    for (jj in 1: nsims) {
		dir.create(file.path(outputfolder, dataset[tt], "data",
                             paste("sim", formatted.count[jj], sep = "_"))) 
		dir.create(file.path(outputfolder, dataset[tt], "reads",
                             paste("sim", formatted.count[jj], sep = "_"))) 
	}
}
...
for (jj in 1: nsims) {
	cat("[EricScript simulator] Generating synthetic dataset",
        formatted.count[jj], "...")
	
	if (flag.background == 1) {
        myrandomseed <- round(runif(1, 1, 1000))
        system(paste("seqtk sample -s", myrandomseed, " background.data_1 ",
                     nreads.background, " > ",
                     file.path(outputfolder, "background.reads.1.fq"),
                     sep = ""))
        system(paste("seqtk sample -s", myrandomseed, " background.data_2 ",
                     nreads.background, " > ",
                     file.path(outputfolder, "background.reads.2.fq"),
                     sep = ""))
	}
	...
	while (flag == 1) {
		...
		if (length(unique(ix.gene12)) == 2 * ngenefusion &
            length(unique(GeneNames[ix.gene12])) == 2 * ngenefusion) {
			flag  <- 0
		} else {
            flag  <- 1
        }
	...
	if ("BE" %in% dataset) {
		...
		for (i in 1: length(sequence1)) {
			...
			sequence.fusions[i] <- paste(substr(sequence1[i], 1, junction1),
                                         substr(sequence2[i], junction2 + 1,
                                         nchar(sequence2[i])), sep = "")
			sequence.fusions.50bp[i] <- paste(substr(sequence1[i],
                                                    (junction1 - 49),
                                                     junction1),
                                              substr(sequence2[i],
                                                     junction2 + 1,
                                                    (junction2 + 50)),
                                              sep = "")
			id.fusions[i] <- paste(">", paste(gene1[i], gene2[i],
                                              sep = "----"), sep = "")
			myref         <- c(myref, c(id.fusions[i], sequence.fusions[i]))
			myref.single  <- c(id.fusions[i], formatfasta(sequence.fusions[i]))
			cat(myref.single, file = file.path(outputfolder, "BE", "data",
                paste("sim", formatted.count[jj], sep = "_"),
                paste("myref", formatted.count.fusions[i], ".fa", sep = "")),
                sep = "\n")
		}
		...
		names(GeneFusions) <- c("gene1", "gene2", "junction1", "junction2",
                                "junctionseq", "coverage", "trans1", "trans2")
		save(GeneFusions, file = file.path(outputfolder, "BE", "data",
             paste("sim", formatted.count[jj], sep = "_"),
                   "GeneFusions.RData"))
		
		system(paste(">", file.path(outputfolder, "BE", "reads",
               paste("sim", formatted.count[jj], sep = "_"),
                     "fusions.reads.1.fq")))
		system(paste(">", file.path(outputfolder, "BE", "reads",
               paste("sim", formatted.count[jj], sep = "_"),
		...
		for (i in 1: ngenefusion) {
			mynreads <- round(mycoverage[i] * nchar(sequence.fusions[i]) /
                             (2*readlength))
			if (verbose == 0) {
				system(paste("wgsim -d ", ins.size, " -r 0.0001 -R 0.001 -s ",
                             sd.inssize, " -N ", mynreads, " -1 ", readlength,
                             " -2 ", readlength," ",
                             file.path(outputfolder, "BE", "data",
                             paste("sim", formatted.count[jj], sep = "_"),
                             paste("myref", formatted.count.fusions[i],
                                   ".fa", sep = "")),
                             " " ,file.path(outputfolder, "BE", "data",
                             paste("sim", formatted.count[jj], sep = "_"),
                                   "out.reads.1.fq")," ",
                             ...
                             paste("sim", formatted.count[jj], sep = "_"),
                                   "out.reads.2.fq"), " 2>> ",
                             file.path(outputfolder, "wgsim.log"), " 1>> ",
                             file.path(outputfolder, "wgsim.log"), sep = ""))
			} else {
				system(paste("wgsim -d ", ins.size, " -r 0.0001 -R 0.001 -s ",
                             sd.inssize, " -N ", mynreads, " -1 ", readlength,
                             " -2 ", readlength," ",
			...
			system(paste("cat", file.path(outputfolder, "BE", "data",
                   paste("sim", formatted.count[jj], sep = "_"),
                         "out.reads.1.fq"), ">>",
                   file.path(outputfolder, "BE", "reads",
                   paste("sim", formatted.count[jj], sep = "_"),
                         "fusions.reads.1.fq")))
			system(paste("cat", file.path(outputfolder, "BE", "data",
                   paste("sim", formatted.count[jj], sep = "_"),
		...
		if (flag.background == 1) {
			system(paste("cat ", file.path(outputfolder, "BE", "reads",
                   paste("sim", formatted.count[jj], sep = "_"),
                         "fusions.reads.1.fq"), " ",
                   file.path(outputfolder, "background.reads.1.fq"), " > ",
                   file.path(outputfolder, "BE", "reads",
                   paste("sim", formatted.count[jj], sep = "_"),
                         "total.reads.1.fq"), sep = ""))
			system(paste("cat ", file.path(outputfolder, "BE", "reads",
                   paste("sim", formatted.count[jj], sep = "_"),
                         "fusions.reads.2.fq"), " ",
                   file.path(outputfolder, "background.reads.2.fq"), " > ",
                   file.path(outputfolder, "BE", "reads",
                   paste("sim", formatted.count[jj], sep = "_"),
                         "total.reads.2.fq"), sep = ""))
		}
		system(paste("rm", file.path(outputfolder, "BE", "data",
               paste("sim", formatted.count[jj], sep = "_"),
                     "out.reads.1.fq"))) 
		system(paste("rm", file.path(outputfolder, "BE", "data",
               paste("sim", formatted.count[jj], sep = "_"),
	...
	if ("IE" %in% dataset) {		
		...
		for (i in 1: length(sequence1)) {
			...
			start.exons <- as.numeric(unlist(strsplit(
                            as.character(Gene.Table[ix.genename.table, 7]),
                            ",")))
			end.exons   <- as.numeric(unlist(strsplit(
                            as.character(Gene.Table[ix.genename.table, 8]),
                            ",")))
			...
			junction2.tot <- c(junction2.tot, junction2)
			sequence.fusions[i]      <- paste(substr(sequence1[i], 1,
                                                     junction1),
                                              substr(sequence2[i],
                                                     junction2 + 1,
                                              nchar(sequence2[i])),
			...
			myref         <- c(myref, c(id.fusions[i], sequence.fusions[i]))
			myref.single  <- c(id.fusions[i], formatfasta(sequence.fusions[i]))
			cat(myref.single, file = file.path(outputfolder, "IE", "data",
                paste("sim", formatted.count[jj], sep = "_"),
                paste("myref", formatted.count.fusions[i], ".fa", sep = "")),
                sep = "\n")
		}
		...
		names(GeneFusions) <- c("gene1", "gene2", "junction1", "junction2",
                                "junctionseq", "coverage", "trans1", "trans2")
		save(GeneFusions, file = file.path(outputfolder, "IE", "data",
             paste("sim", formatted.count[jj], sep = "_"),
                   "GeneFusions.RData"))

		system(paste(">", file.path(outputfolder, "IE", "reads",
               paste("sim", formatted.count[jj], sep = "_"),
                     "fusions.reads.1.fq")))
		system(paste(">", file.path(outputfolder, "IE", "reads",
               paste("sim", formatted.count[jj], sep = "_"),
                     "fusions.reads.2.fq")))
		for (i in 1: ngenefusion) {
			mynreads <- round(mycoverage[i] *
                        nchar(sequence.fusions[i]) / (2 * readlength))
			if (verbose == 0) {
				system(paste("wgsim -d ", ins.size, " -r 0.0001 -R 0.001 -s ",
                             sd.inssize, " -N ", mynreads, " -1 ", readlength,
                             " -2 ", readlength," ",
                             file.path(outputfolder, "IE", "data",
                             paste("sim", formatted.count[jj], sep = "_"),
                             paste("myref", formatted.count.fusions[i], ".fa",
                                   sep = "")), " " ,
                             file.path(outputfolder, "IE", "data",
                             paste("sim", formatted.count[jj], sep = "_"),
                                   "out.reads.1.fq")," ",
			...
			system(paste("cat", file.path(outputfolder, "IE", "data",
                   paste("sim", formatted.count[jj], sep = "_"),
                         "out.reads.1.fq"), ">>",
                   file.path(outputfolder, "IE", "reads",
                   paste("sim", formatted.count[jj], sep = "_"),
                         "fusions.reads.1.fq")))
			system(paste("cat", file.path(outputfolder, "IE", "data",
                   paste("sim", formatted.count[jj], sep = "_"),
		...
		if (flag.background == 1) {
			system(paste("cat ", file.path(outputfolder, "IE", "reads",
                   paste("sim", formatted.count[jj], sep = "_"),
                         "fusions.reads.1.fq"), " ",
                   file.path(outputfolder, "background.reads.1.fq"), " > ",
                   file.path(outputfolder, "IE", "reads",
		...
		system(paste("rm", file.path(outputfolder, "IE", "data",
               paste("sim", formatted.count[jj], sep = "_"),
                     "out.reads.1.fq"))) 
		system(paste("rm", file.path(outputfolder, "IE", "data",
               paste("sim", formatted.count[jj], sep = "_"),
