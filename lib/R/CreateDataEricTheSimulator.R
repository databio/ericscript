...
xx <-  read.delim(file.path(tmpfolder, "transcripts.txt"),
                  sep = "\t", header = F)
geneid       <- xx[[1]]
transcriptid <- xx[[2]]
exonstart    <- xx[[3]]
...
xxseq  <- scan(file.path(tmpfolder, "transcripts.fa"),
               what = list(seq="", id=""), sep = "\t", quiet = T)
seqtmp <- xxseq[[1]]
...
for (i in 1: length(unique.transcriptid)) {
  ...
  EnsemblGene.Structures <- rbind(EnsemblGene.Structures,
                                c(unique.transcriptid[i],
                                  mychr,
                                  mystrand,
                                  genestart,
                                  geneend,
...
colnames(EnsemblGene.Structures) <- c("EnsemblGene",
                                      "Chromosome",
                                      "Strand",
                                      "geneStart",
                                      "geneEnd",
...
save(EnsemblGene.Structures, GeneNames, sequences,
     file = file.path(dbfolder,
                      "data",
                      refid,
