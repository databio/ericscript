...
x <- read.delim(file.path(outputfolder,"out",
                paste(samplename, ".checkselfhomology.blat", sep = "")),
                sep = "\t", header = F)
query.fa <- readLines(file.path(outputfolder,"out",
                      paste(samplename, ".checkselfhomology.fa", sep = "")))
id.fa    <- substr(query.fa[seq(1, length(query.fa), by = 2)], 2,
                   nchar(query.fa[seq(1, length(query.fa), by = 2)]))
...
for (i in 1:length(unique.ids.nolabel)) {
	...
	ix.ab <- c(ix.a, ix.b)
	if (((length(ix.a) > 0) & (length(ix.b) > 0)) | (length(ix.a) > 1 &
          length(ix.b) == 1) | (length(ix.a) == 1 & length(ix.b) > 1)) {
		if ((length(ix.a) > 1 & length(ix.b) == 1)) {
			myflag <- 
                length(which(start.match.tmp[ix.a] %in%
                c((start.match.tmp[ix.b]-3):(start.match.tmp[ix.b]+3)))) +
                length(which(end.match.tmp[ix.a] %in%
                c((end.match.tmp[ix.b]-3):(end.match.tmp[ix.b]+3))))
		} else if ((length(ix.a) == 1 & length(ix.b) > 1)) {
			myflag <- 
                length(which(start.match.tmp[ix.b] %in%
                c((start.match.tmp[ix.a]-3):(start.match.tmp[ix.a]+3)))) +
                length(which(end.match.tmp[ix.b] %in%
                c((end.match.tmp[ix.a]-3):(end.match.tmp[ix.a]+3))))
		} else {
			myflag <- 0
		}
		if (myflag == 0) {
			if (max(diff.match.tmp[ix.ab]) < round(0.8*width) &
              ((length(ix.ab) > 2) & any(diff.match.tmp[ix.ab] < 30) |
              (length(ix.ab) == 2))) {
				if (length(ix.ab) != 0) {
					ix.c <- ix.c[-ix.ab]
				}
				...
				if(length(ix.c) != 0) {
					...
					homology.list[[i]]  <- vector("list", length(
                                                  unique.ids.homology))
					for (j in 1:length(unique.ids.homology)) {
						ix.id.homology <- 
                            which(target[ix.c] == unique.ids.homology[j])
						max.match <- max(match[ix.c][ix.id.homology])
						homology.list[[i]][[j]] <- 
                            cbind(unique.ids.homology[j], max.match)
					}
...
ix.junct <- sort(c(which(ix.junct.nohomology == 1),
                   which(ix.junct.homology == 1)))
...
for (i in 1: length(ix.junct)) {
	...
	if (is.null(list.tmp) == F) {
		...
		for (j in 1: n.homo) {
			info.homology.tmp <- paste(info.homology.tmp,
                                 paste(list.tmp[[j]][1,1],
                                 " (",list.tmp[[j]][1,2],"%)", sep = "" ),
                                 sep = "")
...
info.id.and.homology <- cbind(unique.ids.nolabel[ix.junct], info.homology,
                              flag.dup.a[ix.junct], flag.dup.b[ix.junct])
save(info.id.and.homology, file = file.path(outputfolder,"out",
