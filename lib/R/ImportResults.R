...
toens <- function(ericscriptfolder, genename) {	
	load(file.path(ericscriptfolder,
                   "lib",
                   "data",
                   "EnsemblGene.GeneInfo.RData"))
	ensgene <- as.character(EnsemblGene.GeneInfo$EnsemblGene)[which(
               as.character(EnsemblGene.GeneInfo$GeneName) == genename)]
...
Import_ericscript <- function(outputpath) {
	...
	if (length(filename) == 1) {
		xx     <- read.delim(file.path(outputpath, filename),
                             sep = "\t", header = T)
...
Import_defuse <- function(outputpath) {
	...
	if (length(filename) == 1) {	
		...
		for (seqd in 1: dim(xx)[1]) {
			tmp       <-  unlist(strsplit(as.character(
                                 xx$splitr_sequence[seqd]), "|", fixed = T))
			seq[seqd] <- paste(substr(tmp[1], (nchar(tmp[1])-29),
                               nchar(tmp[1])), substr(tmp[2], 1, 30), sep = "")
		}
		algout        <- list()
		algout$gene5  <- gene5
		algout$gene3  <- gene3
...
Import_chimerascan <- function(outputpath) {
	...
	if (length(filename) == 1) {	
		...
		for (i in 1: length(gene1tmp)) {
			if((length(grep(",", gene1tmp[i])) > 0) &
               (length(grep(",", gene2tmp[i])) == 0)) {	
			...
			} else if ((length(grep(",", gene1tmp[i])) == 0) &
                       (length(grep(",", gene2tmp[i])) > 0)) {
...
Import_fusionmap <- function(outputpath) {
	...
	if (length(filename) == 1) {	
		...
		for (ii in 1: length(ix.reverse)) {  
			seqtmp[ix.reverse[ii]] <- convertToComplement(
                                        reverse(seqtmp[ix.reverse[ii]]))
		}
