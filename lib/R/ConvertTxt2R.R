...
xx.strand   <- read.delim(file.path(tmpfolder, "strand.txt"),
                          sep = "\t", header = F)
...
save(EnsemblGene.GenePosition, 
     file = file.path(dbfolder, "data", refid,
                      "EnsemblGene.GenePosition.RData"))

xx <- read.delim(file.path(tmpfolder, "exonstartend.mrg.txt"),
                 sep = "\t", header = F)
...
EnsemblGene.Structures <- 
    cbind(exgeneid[ix.dup], exchr[ix.dup], strand1, start, end,
          excount, exstart, exend)[ix.srt, ]
EnsemblGene.Structures <- data.frame(EnsemblGene.Structures)
names(EnsemblGene.Structures) <- 
    c("EnsemblGene", "Chromosome", "Strand", "geneStart", "geneEnd",
      "exonCount", "exonStart", "exonEnd")
save(EnsemblGene.Structures,
     file = file.path(dbfolder, "data", refid, "EnsemblGene.Structures.RData"))
...
save(EnsemblGene.GeneInfo,
     file = file.path(dbfolder, "data", refid, "EnsemblGene.GeneInfo.RData"))

if (refid == "homo_sapiens") {
    xx <- read.delim(file.path(tmpfolder, "paralogs.fa"),
                     sep = "\t", header = F)
    EnsemblGene.Paralogs <- xx
    names(EnsemblGene.Paralogs) <- c("EnsemblGene", "Paralogs")
    save(EnsemblGene.Paralogs, 
         file = file.path(dbfolder, "data", refid,
