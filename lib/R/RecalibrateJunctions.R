...
for (i in 1: length(readlength)) {
	grep.readlength <- paste(grep.readlength, " -v -e MD:Z:",
                             readlength[i], sep = "")
}
...
TryRecalibration <- function(outputfolder, verbose) {
	...
	if (verbose == 0) {
		x <- system(paste("blat -tileSize=8 -fine",
                          file.path(outputfolder,"out",".tmp.ref.fa"),
                          file.path(outputfolder, "out", ".link"),
                          file.path(outputfolder, "out",
                                    ".recalibrated.junctions.blat"), " 1>> ",
	  	...
	  	x <- system(paste("blat -tileSize=8 -fine",
                          ...
                          file.path(outputfolder, "out", ".link"),
                          file.path(outputfolder, "out",
                                    ".recalibrated.junctions.blat")))
	}
	yy <- readLines(file.path(outputfolder, "out",
                              ".recalibrated.junctions.blat"), n = 6)
	if (length(yy) > 5) {
		xx <- read.delim(file.path(outputfolder, "out",
                                   ".recalibrated.junctions.blat"),
                         sep = "\t", skip = 5, header = F)
		gapsize <- xx[,8]
	}
	if (all(gapsize <= 3) | (length(yy) <= 5)) {
		if (verbose == 0) {
			x <- system(paste("blat -tileSize=8",
                              file.path(outputfolder,"out",".tmp.ref.fa"),
                              file.path(outputfolder, "out", ".link"),
                              file.path(outputfolder, "out",
                                        ".recalibrated.junctions.blat"),
                              " 1>> ", file.path(outputfolder, "out",
                                                 ".ericscript.log")))
		} else {
			x <- system(paste("blat -tileSize=8",
                              file.path(outputfolder,"out",".tmp.ref.fa"),
...
cat(file.path(outputfolder,"out",".tmp.query.fa"),
              file = file.path(outputfolder,"out",".link"))
...
for (i in 1:length(ids_fasta)) {
	...
	x  <- system(paste("samtools view ",
            ...
            paste(samplename,".remap.sorted.bam", sep = "")), " ",
            ids_fasta[i], " | ", grep.readlength,
            " | awk '((($5==0) && ($6==\"*\")) || ($5>=0))' | ",
            "awk '{ print \">\" $1\"_\"$2,\"\\n\"$10}' > ",
            file.path(outputfolder,"out",".tmp.query.fa"),sep = ""))
		...
		if (length(yy) > 5) {
			...
			if (any(gapsize > 3)) {
				...
				if (length(ix.0) > 0) {
					...
					ix.gap.in.junct <- ix.0[which((gapstarts.tmp1 <=
                                                   junction.tmp) &
                                                  (gapstarts.tmp2 >=
                                                  (junction.tmp - 10 + 1)))]
					...
					if( max.rr >= 1) {
						...
						if ((abs(max.a-max.b)/max.a) < 0.31) {
							...
							sequences.recal[i] <- paste(substr(sequences[i], 1,
                                                        recal.left[i]),
                                                        substr(sequences[i],
                                                        recal.right[i],
                                                        nchar(sequences[i])),
...
ids_fasta.recalibrated <- paste(">", ids_fasta, " junction@",
                                junctions.recalibrated, sep = "")
ref.recalibrated <- c()
for (i in 1:length(ids_fasta.recalibrated)) {
	ref.recalibrated <- c(ref.recalibrated, c(ids_fasta.recalibrated[i],
                          formatfasta(sequences.recal[i])))
}
write(ref.recalibrated, file = file.path(outputfolder,"out",
      paste(samplename,".EricScript.junctions.recalibrated.fa", sep = "")),
      ncolumns = 1, sep = "")
Recalibrated.Data <- cbind(recal.left, recal.right,
                           junctions, count.recal, count.total)
colnames(Recalibrated.Data) <- c("Left_Junction", "Right_Junction",
                                 "Junction", "Recal_Count", "Total_Count")
save(sequences.recal, file = file.path(outputfolder, "out",
     paste(samplename,".sequences.recalibrated.RData", sep = "")))
save(Recalibrated.Data, file = file.path(outputfolder, "out",
     paste(samplename,".Recalibrated.Data.RData", sep = "")))
save(junctions.recalibrated, file = file.path(outputfolder, "out",
