...
filein <- file.path(outputfolder, "out",
                    paste(samplename, ".filtered.out", sep = ""))

xx <- readLines(filein, n = 1)
if (length(xx) == 0) {
  myflag <- 0
  cat(myflag, file = file.path(outputfolder, "out", ".ericscript.flag"))
  stop("No lines available in ", filein,
       ". No discordant reads found with MAPQ set to ", MAPQ,
       ". Try to decrease MAPQ parameter and run again EricScript. Exit!")
} else {
  myflag <- 1
  cat(myflag, file = file.path(outputfolder, "out", ".ericscript.flag"))
}
...
for (i in 1: length(generef)) {
  ...
  tmp        <- sort(summary(as.factor(id_2[ix.gene]),
                             maxsum = length(unique(id_2[ix.gene]))),
                             decreasing = T)
  tmp.genename <- names(tmp)
  tmp.weight   <- as.numeric(tmp)
  if ((max(tmp.weight) >= minreads) &
      (length(which(tmp.weight >= minreads)) <= 10)) {
...
if (file.exists(file.path(dbfolder,"data", refid,
  ...
  if (length(id1) == 0) {
    myflag <- 0
    cat(myflag, file = file.path(outputfolder, "out", ".ericscript.flag"))
    stop("No discordant reads found with minimum reads set to ", minreads,
         ". Exit!")
  }
...
save(MyGF, file = file.path(outputfolder, "out",
